...
$query = "SELECT tanggal_pembayaran FROM riwayat_pembelian WHERE status_bayar = 1 GROUP BY YEAR(tanggal_pembayaran)";
$filter_query = 'SELECT tanggal_pembayaran FROM riwayat_pembelian WHERE status_bayar = 1 GROUP BY YEAR(tanggal_pembayaran) ORDER BY tanggal_pembayaran DESC LIMIT '.$start.', '.$limit.'';
if($_POST['query'] != ''){
  $query = 'SELECT tanggal_pembayaran FROM riwayat_pembelian WHERE tanggal_pembayaran LIKE "%'.$_POST['query'].'%" AND status_bayar = 1 GROUP BY YEAR(tanggal_pembayaran)';//we rasa ada yag salah disini
  $filter_query = 'SELECT tanggal_pembayaran FROM riwayat_pembelian WHERE tanggal_pembayaran LIKE "%'.$_POST['query'].'%" AND status_bayar = 1 GROUP BY YEAR(tanggal_pembayaran) ORDER BY tanggal_pembayaran DESC LIMIT '.$start.', '.$limit.'';
}
