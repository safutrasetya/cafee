...
//ENDHALAMAN
  ...
  $output = '<p class="text-center">TOTAL MEJA : '.$total_data.'</p>
  ...
  if($total_data>0){
    while($row=mysqli_fetch_array($statement)){
      ...
      ';
      if ($row["reservasi"]==0){
        $output .= '<p class="text-success">Kosong</p>';
      }elseif($row["reservasi"]==1){
        $output .= '<p class="text-danger">Penuh</p>';
      }elseif($row["reservasi"]==2){
        $output .= '<p class="text-success">Telah Dibooking</p>';
      }
      $output .= '
        </td>
        <td>
